...
export class AuthService {
    ...
    return new Promise((resolve, reject) => {
      ...
      (
        passport.authenticate(
          'ldapauth',
          { session: false },
          (err: Error | null, user: { uid: string; cn: string; mail: string } | false) => {
            if (err) return reject(err);
            if (!user) return reject(new Error('Credenciais inválidas'));
            resolve({ id: user.uid, nome: user.cn, email: user.mail });
          },
        ) as (req: unknown) => void
      )(fakeReq);
    });
  ...
  async fazerLogin(userId: string, senha?: string): Promise<{ token: string; usuario: object }> {
    ...
    if (ldapEnabled) {
      ...
      console.warn('ATENÇÃO: LDAP desabilitado - autenticação sem validação de senha');
