...
export class PermissaoGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (gruposIds?.length) {
      const grupos = await this.permissoesService.buscarGruposDoUsuario(usuario.id);
    ...
    if (permissoesIds?.length) {
      const permissoes = await this.permissoesService.buscarPermissoesDoUsuario(usuario.id);
